...
class SensorManager:
    ...
    def init_sensor(self, sensor_type, transform, attached, sensor_options):
        if sensor_type == 'RGBCamera':
            ...
            camera = self.world.spawn_actor(camera_bp, transform, attach_to=attached)
            camera.listen(self.save_lane_image)
    ...
    def save_lane_image(self, image):
        ...
        array = array[:, :, :3]
        #array_can = canyEdgeDetector(array)
        array_can = array
        # array = detect(model, array)
        #cv2.imwrite('./traffic-images/t-'+str(self.tics_processing)+'.jpg', array)
        array_can = cv2.cvtColor(array_can, cv2.COLOR_BGR2RGB)
        x = image.width
        y = image.height
        x1 = x//2+20
        y1 = y//3-30
        ...
        x,y,w,h = rect
        croped = array_can[y:y+h, x:x+w].copy()
        bg = array[y:y+h, x:x+w].copy()
        bg = canyEdgeDetector(bg)
        ...
        print(array[200][100])
        self.surface = pygame.surfarray.make_surface(bg.swapaxes(0, 1))
...
def run_simulation(args, client):
    ...
    try:
        ...
        spawn_points = [26,45, 5, 12, 20, 25]
        for i in range(0,len(spawn_points)):
        # Instanciating the vehicle to which we attached the sensors
            bp = blueprint_library.filter('model3')[0]
            bp.set_attribute('role_name', 'hero')
            spawn_point = world.get_map().get_spawn_points()[spawn_points[i]]
